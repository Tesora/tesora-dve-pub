...
public class SchemaTest extends PETest {
	...
	protected String generateCreateTable(UserTable ut) throws PEException {
        ...
        createSql.append("TABLE ")
				.append(Singletons.require(DBNative.class).getNameForQuery(ut))
				.append(" (");
		...
		for ( UserColumn uc : ut.getUserColumns() ) {
            ...
            createSql.append(Singletons.require(DBNative.class).getNameForQuery(uc));

            createSql.append(' ').append(Singletons.require(DBNative.class).getDataTypeForQuery(uc));
	...
	protected static Pair<String, String> getDefaultCharSetAndCollationNames() {
		final DBNative dbHost = Singletons.require(DBNative.class);
