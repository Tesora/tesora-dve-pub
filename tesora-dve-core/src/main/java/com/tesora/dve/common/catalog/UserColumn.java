...
public class UserColumn implements CatalogEntity, PersistentColumn {
	...
	public Boolean defaultValueIsConstant() {
		return ColumnAttributes.isSet(flags, ColumnAttributes.CONSTANT_DEFAULT_VALUE);
	}
	
	public void setDefaultValueIsConstant(Boolean isConstant) {
		if (isConstant == null || Boolean.FALSE.equals(isConstant))
			flags = ColumnAttributes.clear(flags,ColumnAttributes.CONSTANT_DEFAULT_VALUE);
		else
			flags = ColumnAttributes.set(flags, ColumnAttributes.CONSTANT_DEFAULT_VALUE);
	}
